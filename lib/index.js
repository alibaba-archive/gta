(function(root, factory) {
  ...
  'use strict';
  var $body, Providers, checkScript, formatUser, getScript, gta, initGta, newGtaReg, providers, removeElement, slice;
    ...
    customer: function(account) {
      ...
      return {
        ...
        setUser: function(id, user) {
          ...
          user.created_at = new Date(user.created_at).valueOf();
  ...
  formatUser = function(provider, user) {
    ...
    for (key in user) {
      value = user[key];
      if ((_ref = value.alias) != null ? _ref[provider.name] : void 0) {
        result[value.alias[provider.name]] = value.value;
      } else {
        result[key] = value.value != null ? value.value : value;
      }
    }
    return result;
  };
    ...
    setUser: function(id, user) {
      ...
      try {
        ...
        for (_i = 0, _len = providers.length; _i < _len; _i++) {
          ...
          if (this.debug) {
            console.log('formatUser', formatUser(provider, user));
          }
